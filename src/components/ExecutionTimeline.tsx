...
export function ExecutionTimeline() {
  ...
  return (
    <div className="relative shrink-0 border-t border-slate-800/80 bg-ink-900/70">
      {/* 上边缘拖拽手柄：上下调整时间轴高度 */}
      {!collapsed && (
        <div
          onMouseDown={onResizeStart}
          title="拖拽调整高度"
          className="group absolute inset-x-0 top-0 z-20 h-1.5 cursor-row-resize"
        >
      ...
      >
        ...
        <span className="text-[10px] text-slate-600">
          · 共 {timeline.length} 条
          {collapsed ? '（已收起）' : ' · 点击 ◆ Checkpoint 可回溯 · 拖顶边调高'}
        </span>
        {collapsed ? (
      ...
      {!collapsed && (
        <div
          ref={scrollRef}
          style={{ height: size }}
          className="overflow-auto px-4 pb-3 scrollbar-thin"
        >
          <div className="relative flex min-w-max items-start gap-0 pt-1">
            {/* 连接线 */}
            <div className="absolute left-4 right-4 top-[18px] h-px bg-slate-700/80" aria-hidden />
...
function TimelineItem({
  ...
  return (
    ...
    >
      ...
      {/* 时间 */}
      <span className="mt-1 font-mono text-[10px] text-slate-600">{event.time}</span>
      ...
      >
        <div className="truncate text-[10px] font-medium text-slate-400">{event.source}</div>
        <p className={cn('mt-0.5 line-clamp-2 text-[10px] leading-snug', levelText[event.level])}>
          {event.text}
        </p>
      </div>
